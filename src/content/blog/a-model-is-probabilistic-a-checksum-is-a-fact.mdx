...
- **The label instead of the value.** The document says `Tax ID: 12345678901`, and the model returns a detection covering the words *Tax ID* while missing the number. This is the most dangerous failure a privacy tool can have, because the UI looks like it's working. Something got detected. It's just the wrong something, and the actual identifier ships unredacted.
...
- **Real identifiers got the wrong label.** An IBAN, which has a checksum and is *verifiable*, came back as a "reference number". A BIC came back as a person's name.
...
A checksum, by contrast, is exact. An IBAN either passes the mod-97 check or it doesn't, with no confidence score involved. So I made the deterministic recognizers authoritative and treated the model's output as a proposal to be checked against them. Here's the reconciler:
...
The label-driven and `NSDataDetector` recognizers are loose. On an invoice they were overriding correct model detections, a real name or a real street address, with their own worse reading of the same span. They were winning arguments they should have lost.
...
An 8-page NDA produced **199 detections**. Almost all of it was noise: `customers`, `software`, `third party`, `and`, `its`. Capitalised boilerplate like `Party A`, `Receiving Party`, `Disclosing Party`. Pronouns like `you`, `your`, `us`.
...
German capitalises every noun. `Geburtsname`, `Anschrift`, `Arbeitnehmer`, `Nein`: all capitalised, and all sail straight through the rule I'd just written.
...
A single-token span immediately followed by a colon is a form **caption**, never the value. `Straße:`, `Geburtsname:`, `Ortsteil:`. It's language-agnostic, because it keys on punctuation, not words. The single-token requirement is what makes it safe: a real multi-word address can't be mistaken for a caption, because it contains spaces.
...
A PDF's info dictionary routinely carries the author and title, and an official letter's title often contains the recipient's name. The preview never shows it. So document attributes are stripped on every export, in both modes, and the "nothing was selected" path re-serialises the stripped copy rather than handing back the original bytes.

### The failure I think about most

PDFKit *silently refuses* to mutate a permission-locked PDF. No error, no exception: the calls just don't do anything. Which meant that for encrypted documents, the export re-emitted the original file, unredacted and with its metadata intact. The code was correct. The API said nothing. The output was a document the user believed was safe.

The fix is upstream. Encrypted PDFs are normalised to a clean, mutable copy at import, so the exporter never sees a locked document. But the lesson generalises past PDFKit: in a privacy tool, an operation that quietly does nothing is worse than one that crashes. A crash is visible; this one was silent, and it produced a file whose entire purpose was a promise it wasn't keeping.
...
[Veil is on the App Store](https://apps.apple.com/us/app/veil-redact-hide-info/id6780531974), and everything above runs entirely on the phone.
